...
| `internal/search/recall_test.go` | Unit tests for RRF, FTS-only fallback, boost_verified, graph seeds, empty query, configurable k, scope-filter error guard |
...
- Hands-on delivery: branch must be rebased onto current `origin/main` (not stale fork branch); exclude unrelated template/mkdocs commits from the PR (14 knowledge/research/runbook template files removed from `feat/kiwi-recall-422`).
