...
type Scanner struct {
	...
	externalLinkCheck  *ExternalLinkCheckConfig
}
...
func (s *Scanner) Scan(ctx context.Context) (*ScanResult, error) {
	...
	if extFindings, err := s.checkExternalLinks(ctx, pages); err != nil {
		return nil, err
	} else if len(extFindings) > 0 {
		result.ExternalLinks = extFindings
		result.Issues = append(result.Issues, externalLinkFindingsToIssues(extFindings)...)
	}
