...
func TestInitADRIntoEmptyParent(t *testing.T) {
	t.Parallel()
	parent := t.TempDir()
	root := filepath.Join(parent, "nested", "adr")
	if err := Init(root, "adr"); err != nil {
		t.Fatal(err)
	}
...
func TestInitADRDoesNotOverwriteExisting(t *testing.T) {
	t.Parallel()
...
func TestADRSchemaRejectsInvalidFrontmatter(t *testing.T) {
	t.Parallel()
	root := filepath.Join(t.TempDir(), "adr-schema")
	if err := Init(root, "adr"); err != nil {
		t.Fatal(err)
	...
	cases := []struct {
	...
	}{
		{
			name: "missing title",
			fm: map[string]any{
				"type": "adr", "status": "proposed", "date": "2026-06-19",
				"deciders": []any{"team-a"},
			},
		},
		{
			name: "missing status",
			fm: map[string]any{
				"type": "adr", "title": "ADR", "date": "2026-06-19",
				"deciders": []any{"team-a"},
			},
		},
		{
			name: "invalid status",
			fm: map[string]any{
				"type": "adr", "title": "ADR", "status": "draft", "date": "2026-06-19",
				"deciders": []any{"team-a"},
			},
		},
		{
			name: "invalid state",
			fm: map[string]any{
				"type": "adr", "title": "ADR", "status": "proposed", "state": "draft",
				"date": "2026-06-19", "deciders": []any{"team-a"},
			},
		},
		{
			name: "invalid workflow",
			fm: map[string]any{
				"type": "adr", "title": "ADR", "status": "proposed", "workflow": "tasks",
				"date": "2026-06-19", "deciders": []any{"team-a"},
			},
		},
		{
			...
			fm: map[string]any{
				"type": "adr", "title": "ADR", "status": "proposed", "date": "06/19/2026",
				"deciders": []any{"team-a"},
			},
		},
		{
			name: "empty deciders",
			fm: map[string]any{
				"type": "adr", "title": "ADR", "status": "proposed", "date": "2026-06-19",
				"deciders": []any{},
			},
		},
	}
...
func TestADRConfigHasAuthGuidance(t *testing.T) {
	t.Parallel()
	root := filepath.Join(t.TempDir(), "adr-config")
	if err := Init(root, "adr"); err != nil {
		t.Fatal(err)
	}
...
func TestBlankADRTemplateHasPlaceholderDeciders(t *testing.T) {
	t.Parallel()
	data, err := templates.ReadFile("templates/adr/.kiwi/templates/adr.md")
	if err != nil {
		t.Fatal(err)
	}
	if strings.Contains(string(data), "deciders: []") {
		t.Fatal("blank ADR template must not use empty deciders array; use placeholder")
	}
	if !strings.Contains(string(data), "deciders: [team-or-person]") {
		t.Fatal("blank ADR template should include deciders placeholder")
	}
}
