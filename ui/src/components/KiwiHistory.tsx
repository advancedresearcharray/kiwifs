...
import { GitBranch, History, RotateCcw, User, X, Columns2 } from "lucide-react";
...
type Props = {
  ...
  onClose: () => void;
  onRestored?: () => void;
  onCompareWithCurrent?: (versionHash: string) => void;
};
    ...
    <div className="flex flex-col h-full">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-8 py-3 border-b border-border">
        ...
        <div className="flex items-center gap-2">
          {selectedHash && versions && versions.length > 0 && selectedHash !== versions[0].hash && onCompareWithCurrent && (
            <Button
              variant="outline"
              size="sm"
              onClick={() => onCompareWithCurrent(selectedHash)}
            >
              <Columns2 className="h-3.5 w-3.5" /> Compare with current
            </Button>
          )}
